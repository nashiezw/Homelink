...
const fieldShell =
  "grid min-w-0 gap-1 rounded-md border border-slate-200 bg-white px-3 py-2 transition focus-within:border-emerald-400 focus-within:ring-2 focus-within:ring-emerald-500/15 dark:border-slate-700 dark:bg-slate-950/60";
...
export function SearchBar({ compact = false, initialLocation = "", initialIntent = "rent" }: SearchBarProps) {
  ...
  return (
    <form onSubmit={submit} className="surface-panel w-full min-w-0 overflow-hidden rounded-2xl p-4 shadow-hero">
      <div className="grid min-w-0 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <label className={fieldShell}>
          ...
          <div className="flex min-w-0 items-center gap-1.5 text-sm text-slate-800 dark:text-slate-100">
        ...
        <label className={fieldShell}>
          ...
          <select
            className="w-full min-w-0 bg-transparent text-sm text-slate-800 outline-none dark:text-slate-100"
            value={propertyType}
        ...
        <label className={fieldShell}>
          ...
          <select
            className="w-full bg-transparent text-sm text-slate-800 outline-none disabled:opacity-60 dark:text-slate-100"
            value={intent}
      ...
      <div className={`mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between ${compact ? "" : ""}`}>
        {!compact ? (
          <div className="flex min-w-0 flex-1 flex-wrap items-center gap-2">
            ...
            <button
              ...
              className="inline-flex items-center gap-1 rounded-full border border-slate-200 px-3 py-1.5 text-sm font-medium text-ocean hover:border-ocean/40 hover:bg-cyan-50"
            >
        ...
        <Button type="submit" className="h-12 w-full shrink-0 sm:w-auto sm:min-w-[12.5rem]">
