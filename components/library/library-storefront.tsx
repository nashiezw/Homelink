    ...
    <main className="bg-[#f5f7f4] text-ink dark:bg-slate-950 dark:text-white">
      ...
      <section className="relative overflow-hidden border-b border-emerald-900/15 bg-ink text-white">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-10 px-4 py-10 sm:px-6 lg:grid-cols-[minmax(0,1fr)_31rem] lg:items-center lg:px-8 lg:py-14">
          <div className="min-w-0">
            <div className="inline-flex items-center gap-2 rounded-full border border-emerald-200/25 bg-white/10 px-3 py-1 text-xs font-bold uppercase text-emerald-100 backdrop-blur"><LibraryBig className="size-4" /> HouseLink Library</div>
            <h1 className="mt-5 max-w-4xl text-[2.6rem] font-semibold leading-[1.02] tracking-normal text-white sm:text-6xl lg:text-[4.8rem]">The professional property bookshelf for Zimbabwe.</h1>
            <p className="mt-5 max-w-2xl text-base leading-8 text-slate-200 sm:text-lg">Books, manuals, legal packs, templates, toolkits, and course-ready resources built directly into the HouseLink ecosystem.</p>
            <div className="mt-7 flex flex-col gap-3 sm:flex-row">
              <Link href="#library-products" className="inline-flex h-12 items-center justify-center gap-2 rounded-lg bg-emerald-500 px-5 text-sm font-bold text-slate-950 shadow-xl shadow-emerald-950/30 transition hover:bg-emerald-400">Explore shelves <ArrowRight className="size-4" /></Link>
              <Link href="/dashboard/my-library" className="inline-flex h-12 items-center justify-center gap-2 rounded-lg border border-white/20 bg-white/10 px-5 text-sm font-bold text-white backdrop-blur transition hover:bg-white/15">My Library <BookOpen className="size-4" /></Link>
            </div>
            <div className="mt-8 grid max-w-3xl gap-2 sm:grid-cols-3">
              <Proof icon={ShieldCheck} label="Tracked links" />
              <Proof icon={Download} label="PDF, ZIP, DOCX" />
      ...
      <section className="border-b border-slate-200 bg-white/90 dark:border-slate-800 dark:bg-slate-950/90">
        <div className="mx-auto grid max-w-7xl gap-3 px-4 py-4 sm:px-6 lg:grid-cols-[minmax(0,1fr)_auto_auto_auto] lg:items-center lg:px-8">
          <label className="relative min-w-0"><Search className="pointer-events-none absolute left-4 top-1/2 size-4 -translate-y-1/2 text-slate-400" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search title, author, ISBN, contract, course, manual..." className="h-12 w-full rounded-lg border border-slate-200 bg-white pl-11 pr-4 text-sm outline-none transition focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 dark:border-slate-700 dark:bg-slate-900" /></label>
      ...
      <section className="mx-auto grid max-w-7xl gap-7 px-4 py-8 sm:px-6 lg:grid-cols-[minmax(0,1fr)_22rem] lg:px-8">
        <div id="library-products" className="min-w-0 space-y-10">
          ...
          <section className="space-y-4">
            <div className="flex items-end justify-between gap-4"><div><p className="text-sm font-bold uppercase text-emerald-700 dark:text-emerald-300">Complete Catalogue</p><h2 className="text-2xl font-semibold tracking-normal text-ink dark:text-white">{results.length} Library products</h2></div></div>
        ...
        <aside className="h-fit space-y-4 lg:sticky lg:top-24">
          ...
          <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-950 shadow-sm dark:border-emerald-900/40 dark:bg-emerald-950/20 dark:text-emerald-100"><p className="font-bold">Built for professional publishing</p><div className="mt-3 space-y-2">{["Secure links, license keys and download logs", "Invoices, fulfilment, coupons and tax settings", "Native HouseLink payments and customer accounts"].map((item) => <p key={item} className="flex gap-2"><CheckCircle2 className="mt-0.5 size-4 shrink-0" /> {item}</p>)}</div></div>
        </aside>
      </section>
    </main>
  );
...
function HeroProduct({ product, quantity, onAdd }: { product: LibraryProduct; quantity: number; onAdd: (product: LibraryProduct) => void }) {
  return <div className="grid gap-4 rounded-lg border border-white/15 bg-white/[0.08] p-4 shadow-2xl shadow-slate-950/40 backdrop-blur sm:grid-cols-[12rem_minmax(0,1fr)]"><Cover product={product} /><div className="min-w-0 self-center"><p className="inline-flex rounded-full bg-emerald-300 px-2.5 py-1 text-xs font-black uppercase text-slate-950">Editor's choice</p><Link href={`/library/${product.slug}`} className="mt-3 block text-2xl font-semibold leading-tight text-white hover:text-emerald-200">{product.title}</Link><p className="mt-2 line-clamp-3 text-sm leading-6 text-slate-300">{product.shortDescription}</p><p className="mt-4 text-2xl font-black text-white">USD {product.price.toFixed(2)}</p><div className="mt-4 flex flex-col gap-2 sm:flex-row"><Button onClick={() => onAdd(product)}><ShoppingCart className="size-4" /> {quantity ? `In bag (${quantity})` : "Add to cart"}</Button><Link href={`/library/${product.slug}`} className="inline-flex h-10 items-center justify-center rounded-lg border border-white/20 px-4 text-sm font-bold text-white hover:bg-white/10">View details</Link></div></div></div>;
}
...
function ProductCard({ product, quantity, onAdd }: { product: LibraryProduct; quantity: number; onAdd: (product: LibraryProduct) => void }) {
  return <article className="group overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl dark:border-slate-800 dark:bg-slate-900"><Cover product={product} /><div className="flex min-w-0 flex-col p-4"><p className="text-xs font-black uppercase text-emerald-700 dark:text-emerald-300">{product.productType.replace(/_/g, " ")}</p><Link href={`/library/${product.slug}`} className="mt-1 line-clamp-2 text-lg font-semibold leading-snug text-ink hover:text-emerald-700 dark:text-white">{product.title}</Link><p className="mt-1 text-sm text-slate-500">{product.author}</p><p className="mt-3 line-clamp-2 text-sm leading-6 text-slate-600 dark:text-slate-300">{product.shortDescription}</p><div className="mt-4 flex items-center justify-between gap-3"><span className="flex items-center gap-1 text-sm text-amber-500"><Star className="size-4 fill-current" /><span className="font-bold text-slate-700 dark:text-slate-200">{product.rating || "New"}</span></span><p className="text-lg font-black text-ink dark:text-white">USD {product.price}</p></div><div className="mt-4 grid grid-cols-[1fr_auto] gap-2"><Button onClick={() => onAdd(product)} disabled={product.comingSoon && !product.preorder}><ShoppingCart className="size-4" /> {quantity ? `In bag (${quantity})` : product.preorder ? "Pre-order" : "Add"}</Button><Link href={`/library/${product.slug}`} className="inline-flex size-10 items-center justify-center rounded-lg border border-slate-200 text-slate-600 transition hover:border-emerald-500 hover:text-emerald-700 dark:border-slate-700 dark:text-slate-300" aria-label={`View ${product.title}`}><ArrowRight className="size-4" /></Link></div></div></article>;
}

function Cover({ product }: { product: LibraryProduct }) {
  return <Link href={`/library/${product.slug}`} className="relative block aspect-[4/3] overflow-hidden bg-slate-100 dark:bg-slate-950"><Image src={product.gallery[0]?.url ?? "/images/academy/agent-academy-hero.png"} alt={product.title} fill className="object-cover transition duration-500 group-hover:scale-[1.04]" sizes="(min-width: 1024px) 340px, 100vw" /><div className="absolute inset-x-0 bottom-0 h-20 bg-gradient-to-t from-slate-950/70 to-transparent" /></Link>;
}
...
function CartPanel({ cart, total, currency, onCart }: { cart: LibraryCartLine[]; total: number; currency: string; onCart: (cart: LibraryCartLine[]) => void }) {
  return <div className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-xl dark:border-slate-800 dark:bg-slate-900"><div className="bg-ink p-4 text-white"><p className="flex items-center gap-2 text-sm font-bold"><ShoppingBag className="size-4" /> Library Bag</p><p className="mt-1 text-xs leading-5 text-slate-300">Native checkout, coupons, invoices, and secure delivery.</p></div><div className="space-y-3 p-4">{cart.length ? cart.map((line) => <div key={line.productId} className="rounded-lg border border-slate-200 p-3 text-sm dark:border-slate-700"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="line-clamp-2 font-bold text-ink dark:text-white">{line.title}</p><p className="text-slate-500">Qty {line.quantity}</p></div><p className="font-black">{line.currency} {(line.price * line.quantity).toFixed(2)}</p></div><div className="mt-3 flex items-center justify-between gap-3"><div className="inline-flex items-center rounded-lg border border-slate-200 dark:border-slate-700"><button type="button" onClick={() => onCart(cart.map((item) => item.productId === line.productId ? { ...item, quantity: Math.max(1, item.quantity - 1) } : item))} className="grid size-8 place-items-center text-slate-500 hover:text-emerald-700" aria-label="Decrease quantity"><Minus className="size-3.5" /></button><span className="w-8 text-center text-xs font-black">{line.quantity}</span><button type="button" onClick={() => onCart(cart.map((item) => item.productId === line.productId ? { ...item, quantity: item.quantity + 1 } : item))} className="grid size-8 place-items-center text-slate-500 hover:text-emerald-700" aria-label="Increase quantity"><Plus className="size-3.5" /></button></div><button type="button" onClick={() => onCart(cart.filter((item) => item.productId !== line.productId))} className="inline-flex items-center gap-1 text-xs font-bold text-slate-500 hover:text-red-600"><Trash2 className="size-3.5" /> Remove</button></div></div>) : <div className="grid min-h-32 place-items-center rounded-lg border border-dashed border-slate-300 p-4 text-center text-sm text-slate-500 dark:border-slate-700"><div><ShoppingCart className="mx-auto mb-2 size-6 text-slate-400" />Add a book, manual, template, or toolkit.</div></div>}<div className="flex items-center justify-between border-t border-slate-200 pt-3 font-black dark:border-slate-700"><span>Total</span><span>{currency} {total.toFixed(2)}</span></div><Link href="/library/checkout" className={cn("inline-flex h-10 w-full items-center justify-center gap-2 rounded-lg text-sm font-bold", cart.length ? "bg-emerald-600 text-white hover:bg-emerald-500" : "pointer-events-none bg-slate-200 text-slate-500 dark:bg-slate-800")}><ShoppingCart className="size-4" /> Checkout</Link></div></div>;
}
...
function Proof({ icon: Icon, label }: { icon: typeof ShieldCheck; label: string }) {
  return <div className="rounded-lg border border-white/12 bg-white/10 p-3 backdrop-blur"><Icon className="size-5 text-emerald-300" /><p className="mt-2 text-sm font-bold text-white">{label}</p></div>;
}
