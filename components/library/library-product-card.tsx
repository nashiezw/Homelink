...
function ProductCardPendingOverlay() {
  const { pending } = useLinkStatus();
  return (
    <span
      className={cn(
        "pointer-events-none absolute inset-0 z-20 flex items-center justify-center rounded-2xl bg-white/70 opacity-0 backdrop-blur-[1px] transition-opacity duration-150 dark:bg-slate-950/55",
        pending && "opacity-100 delay-75",
      )}
      aria-hidden={!pending}
    >
      <span className={cn("inline-flex items-center gap-2 rounded-full bg-[#22a54b] px-3 py-1.5 text-xs font-bold text-white shadow-lg opacity-0", pending && "opacity-100")}>
...
export function LibraryProductCard({
  ...
  const href = `/library/${product.slug}`;

  return (
    <article className="group relative flex h-full flex-col rounded-2xl border border-[#dfe8e5] bg-white p-3 shadow-sm transition hover:-translate-y-1 hover:border-[#22a54b] hover:shadow-xl sm:p-4 dark:border-slate-800 dark:bg-slate-900">
      <Link href={href} className="absolute inset-0 z-[1] rounded-2xl" aria-label={`Open ${product.title}`}>
        <ProductCardPendingOverlay />
      </Link>
      ...
      <div className="pointer-events-none relative z-[2] flex min-h-0 flex-1 flex-col">
        ...
        <div className="mt-3 flex min-w-0 flex-1 flex-col sm:mt-4">
          ...
          <p className="mt-1 line-clamp-2 text-sm font-black leading-snug text-[#1a3560] group-hover:text-[#22a54b] sm:text-base dark:text-white">
            {product.title}
          </p>
          ...
          <div className="mt-3 flex items-center justify-between gap-3 sm:mt-4">
            ...
            {hidePrice ? (
              <Link
                ...
                className="pointer-events-auto relative z-[3] text-sm font-bold text-[#22a54b] underline-offset-2 hover:underline"
          ...
          <div className="pointer-events-auto mt-auto grid grid-cols-[1fr_auto] gap-2 pt-3 sm:pt-4">
            <Button
              type="button"
              onClick={(event) => {
              ...
              className="relative z-[3] min-h-11 rounded-full bg-[#22a54b] shadow-md shadow-emerald-900/10 hover:bg-[#1e9443] hover:from-[#22a54b] hover:to-[#22a54b]"
            >
              <ShoppingCart className="size-4" />{" "}
              {quantity ? `In bag (${quantity})` : product.preorder ? "Pre-order" : "Add"}
            </Button>
            <span
              className="inline-flex size-11 items-center justify-center rounded-full border border-slate-200 text-slate-600 shadow-sm transition group-hover:border-[#22a54b] group-hover:text-[#22a54b] dark:border-slate-700 dark:text-slate-300"
              aria-hidden
            >
