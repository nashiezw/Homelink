...
export function LibraryCartFab({ className }: { className?: string }) {
  ...
  // Checkout already shows the full bag — keep the FAB off that page.
  ...
  return (
    <div className={cn("fixed bottom-[max(1.25rem,env(safe-area-inset-bottom))] right-4 z-[60] flex flex-col items-end gap-3 sm:right-5 lg:hidden", className)}>
      {open && (
        <div className="w-[min(22rem,calc(100vw-2rem))] overflow-hidden rounded-2xl border border-emerald-900/10 bg-white shadow-[0_24px_60px_rgba(6,78,59,0.18)] dark:border-emerald-400/15 dark:bg-slate-950">
