...
export function AgentCommissionCalculator() {
  ...
  return (
    <CalculatorCard
      ...
      description="Calculate commission splits between HomeLink agents and the platform."
      audience="Agents"
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(18rem,0.9fr)] lg:gap-8">
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <CalculatorPresetButton active={preset === "60-40"} onClick={() => applyPreset("60-40", "60")}>
                60% Agent / 40% HomeLink
              </CalculatorPresetButton>
              <CalculatorPresetButton active={preset === "40-60"} onClick={() => applyPreset("40-60", "40")}>
                40% Agent / 60% HomeLink
              </CalculatorPresetButton>
              <CalculatorPresetButton active={preset === "custom"} onClick={() => setPreset("custom")}>
                Custom Split
              </CalculatorPresetButton>
            </div>
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            <CalculatorField
              ...
              suffix="%"
              value={agentPercent}
              onChange={handleAgentPercentChange}
            />
            <label htmlFor="commission-homelink" className="block">
              ...
              <div className="relative mt-2">
                <input
                  id="commission-homelink"
                  type="text"
                  readOnly
                  value={String(result.homeLinkPercent)}
                  className="w-full cursor-default rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-base font-medium text-slate-700 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200"
                />
            ...
            <CalculatorResultRow
              ...
              value={
                <AnimatedCurrency
                  ...
                  format={(value) => formatCalculatorCurrency(value, 2)}
                />
              }
              emphasis
            />
        ...
        >
          <CalculatorResultRow
            ...
            value={
              <AnimatedCurrency
                value={result.agentEarnings}
                format={(value) => formatCalculatorCurrency(value, 2)}
              />
            }
          />
          <CalculatorResultRow
            label="HomeLink Earnings"
            value={
              <AnimatedCurrency
                value={result.homeLinkEarnings}
                format={(value) => formatCalculatorCurrency(value, 2)}
              />
            }
          />
