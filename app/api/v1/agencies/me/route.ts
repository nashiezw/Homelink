...
import {
  getAgencyDashboardFromPostgres,
  inviteAgencyAgentInPostgres,
...
export async function GET(request: Request) {
  ...
  if (shouldUsePostgresAgents()) {
    const dashboard = await getAgencyDashboardFromPostgres(userId);
    if (!dashboard) return problem(403, "FORBIDDEN", "Agency admin access required.");
    return ok(dashboard);
  }
...
export async function POST(request: Request) {
  ...
  if (shouldUsePostgresAgents()) {
    ...
    if (!name || !email) return problem(400, "INVALID_INPUT", "Name and email are required.");
    const result = await inviteAgencyAgentInPostgres(userId, { name, email });
    if (result.error) return problem(400, "INVITE_FAILED", result.error);
    const dashboard = await getAgencyDashboardFromPostgres(userId);
