...
import { LiveChatError, makeLiveChatVisitorKey, updateLiveChatActivity } from "@/lib/live-chat/repository";
...
export async function POST(request: Request) {
  try {
    ...
    return ok(await updateLiveChatActivity({
      request,
      visitorKey,
    ...
    console.error("[live-chat/activity] failed", error);
    return problem(500, "LIVE_CHAT_ACTIVITY_FAILED", "Live Chat activity could not be updated.");
  }
}
